...
export default function RoomPage() {
  ...
  // Fetch audio URL when current song changes
  useEffect(() => {
    if (!currentSong) {
      setAudioUrl(null);
    ...
    // Reset time immediately for the new song
    setCurrentTime(0);
    setDuration(0);
    if (audioRef.current) {
    ...
    const fetchAudio = async () => {
      try {
        ...
        setAudioUrl(`/api/youtube/stream/${currentSong.videoId}`);
        setDisplayedSong(currentSong);
        setAudioLoading(false);
      }
    };
  ...
  // Audio sync: when audio URL loaded, start from 0
  useEffect(() => {
